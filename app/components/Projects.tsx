...
export default function Projects({ id, projectName, scrollToShades }: { id: string, projectName: string, scrollToShades: () => void }) {
    ...
    const handleClick = (projectId: string) => {
        router.push(`/home?projectId=${projectId}&scrollTo=shades`);
    }
    ...
    return (
        <div className="flex flex-col justify-between h-full pb-8">
            ...
            <div>
                ...
                <SaveProject projectId={id} projectName={projectNameState} disabled={!projectNameState || !!error} />
                <button onClick={() => {window.location.href = "/home"}} className="w-full bg-black text-lg text-background p-2 font- mt-2 rounded-md cursor-pointer hover:bg-gold-500 hover:text-black hover:shadow-md">
                    New Project
                </button>
